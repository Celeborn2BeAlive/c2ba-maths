...
inline discrete_1d_sample uniformDiscreteSample(float s1D, std::size_t count) {
    return count ? discrete_1d_sample(clamp(std::size_t(s1D * (count - 1)),
                                  std::size_t(0),
                                  count), 1.f / count) : discrete_1d_sample(0, 0.f);
}
